...
4. From commandline, `cd` into the `docker` directory and run `docker-compose up`
	...
	Starting playground-mysql
	Starting playground-mailhog
	Starting playground-php-fpm
	Starting playground-nginx
	Attaching to playground-mailhog, playground-mysql, playground-php-fpm, playground-nginx
	playground-mailhog    | 2017/06/15 16:34:26 Using in-memory storage
	...
	playground-mysql      | 2017-06-15T16:34:27.401334Z 0 [Note] mysqld (mysqld 5.7.17) starting as process 1 ...
	...
	playground-mysql      | 2017-06-15T16:34:27.408857Z 0 [Warning] Setting lower_case_table_names=2 because file system for /var/lib/mysql/ is case insensitive
	...
	playground-mysql      | 2017-06-15T16:34:28.332626Z 0 [Note] mysqld: ready for connections.
	playground-mysql      | Version: '5.7.17'  socket: '/var/run/mysqld/mysqld.sock'  port: 3306  MySQL Community Server (GPL)
	playground-mailhog    | [APIv1] KEEPALIVE /api/v1/events
...
Lastly, **Find/Replace** `playground` with the name of your app.

> **WHY?** by default the files are set to name the containers based on your app prefix. By default this is `playground`.
> A find/replace on `playground` is safe and will allow you to customize the names of the containers
>
> e.g. playground-mysql, playground-php-fpm, playground-nginx, playground-mailhog
...
To change these defaults edit the variables in the `docker/.env` file or tweak the `docker-compose.yml` file under `playground-mysql`'s `environment` section.
...
```bash
docker exec -it playground-php-fpm /bin/bash
```
> remember to replace `playground` with whatever you really named the container

**access mysql cli**

```bash
docker exec -it playground-mysql /usr/bin/mysql -u root -p playground
```
> remember to replace `playground` with whatever you really named the container and with your actual database name and user login
...
### `playground-nginx` - the web server

First we're creating an nginx server. The configuration is set based on the CakePHP suggestions for nginx and `playground-nginx` will handle all the incoming requests from the client and forward them to the `playground-php-fpm` server which is what actually runs your PHP code.

You can configure the **nginx server** by editing the `/nginx/nginx.conf` file

### `playground-php-fpm` - the PHP processor
...
### `playground-mysql` - the database server
...
### `playground-mailhog` - the smtp server
...
`playground-nginx | nginx: [emerg] open() "/var/www/playground/logs/access.log" failed (2: No such file or directory)`

This is caused by not installing CakePHP completely and can be fixed by creating the logs folder in your `playground/cakephp` folder.
...
Most of the time I set this up without an existing CakePHP app. This will cause the initial `up` command to fail because folders are missing. To solve this run the install command (from the official CakePHP docs) but set the app name to `.` instead of `playground`.
